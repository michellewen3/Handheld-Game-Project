...
loop	LDR R1, =SSI0_SR_R			;loop checks bit four, if high, loops back
		LDRB R2, [R1]
		ANDS R2, #0x10
		BNE loop
		...
		LDR R3, =DC
		LDR R4, [R3]	
		AND R4, #0xFFFFFFBF;		;clear PA6
		STR R4, [R3]
		LDR R5, =SSI0_DR_R
		STRB R0, [R5]
		
loop2	LDR R1, =SSI0_SR_R			;loop checks bit four, if high, loops back
		LDR R2, [R1]
		ANDS R2, #0x10
		BNE loop2
